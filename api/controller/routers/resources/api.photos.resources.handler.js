...
router.route("/") .get(async (request, response) => {ReadResources(request, response, "Photo_Resources")}).post(async (request, response) => {
    ...
    try {
      ...
      const PhotoResources = await DBMSPool_connection.query("SELECT * FROM Photo_Resources");
      ...
      let FoundExistingPhotoResource = PhotoResources[0].find((resource) => {
        ...
        } else if (FoundExistingPhotoResource && Object.is(FoundExistingPhotoResource, FoundExistingPhotoResource)) {
...
router.route("/:id").get(async (request, response) => {
  ReadResource(request, response, "Photo_Resources");
      ...
      try {
        ...
        if (request && !FoundExistingPhotoResource || typeof FoundExistingPhotoResource === "undefined") {
          return this.response
            .status(404)
            .json({
          ...
          return this.response
            .status(200)
            .jsonp(
              "body" in request ? { Message: `Updated resource with id ${FoundExistingPhotoResource.id}` } : ""
            );
        }
      } catch (error) {
        // console.log(error);
    ...
    }).delete(async (request, response) => {DeleteResource(request, response, "Photo_Resources")});
