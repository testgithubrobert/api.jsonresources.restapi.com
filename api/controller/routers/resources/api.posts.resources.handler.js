...
router.route("/") .get(async (request, response) => {
    ReadResources(request, response, "Posts_Resources");
    ...
    try {
      ...
      const PostsResources = await DBMSPool_connection.query("SELECT * FROM Posts_Resources");
      ...
      let FoundExistingPostResource = PostsResources[0].find((resource) => {
        ...
        } else if (FoundExistingPostResource && Object.is(FoundExistingPostResource, FoundExistingPostResource)) {
...
router.route("/:id").get(async (request, response) => {
    ReadResource(request, response, "Posts_Resources");
      ...
      try {
        ...
        if (request && !FoundExistingPostResource || typeof FoundExistingPostResource === "undefined") {
          return this.response
            .status(404)
            .json({
        ...
        } else if(typeof request.body.post === "undefined" || typeof request.body.creator === "undefined" || typeof request.body.title === "undefined") {
            return this.response.status(400).sendStatus(400);
        } else {
          ...
          return this.response
            .status(200)
            .jsonp(
              "body" in request ? { Message: `Updated resource with id ${FoundExistingPostResource.id}` } : ""
            );
        }
      } catch (error) {
        // console.log(error);
    ...
    }).delete(async (request, response) => {DeleteResource(request, response, "Posts_Resources")});
