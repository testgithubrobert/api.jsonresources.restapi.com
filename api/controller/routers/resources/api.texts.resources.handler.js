...
router.route("/") .get(async (request, response) => {
    ReadResources(request, response, "Texts_Resources");
    ...
    try {
      ...
      const TextsResources = await DBMSPool_connection.query("SELECT * FROM Texts_Resources");
      ...
      let FoundExistingTextResource = TextsResources[0].find((resource) => {
        ...
        } else if (FoundExistingTextResource && Object.is(FoundExistingTextResource, FoundExistingTextResource)) {
...
router.route("/:id").get(async (request, response) => {ReadResource(request, response, "Texts_Resources")}).put(async (request, response) => {
      this.response = response;
      ...
      try {
        ...
        if (request && !FoundExistingTextResource || typeof FoundExistingTextResource === "undefined") {
          return this.response
            .status(404)
            .json({
          ...
          return this.response
            .status(200)
            .jsonp(
              "body" in request ? { Message: `Updated resource with id ${FoundExistingTextResource.id}` } : ""
            );
        }
      } catch (error) {
        // console.log(error);
    ...
    }).delete(async (request, response) => {DeleteResource(request, response, "Texts_Resources")});
