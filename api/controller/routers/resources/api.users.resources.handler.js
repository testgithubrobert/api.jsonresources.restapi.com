...
router.route("/") .get(async (request, response) => {
    ...
    try {
      ...
      const UsersResources = await DBMSPool_connection.query("SELECT * FROM Users_Resources");
      ...
      let FoundExistingUserResource = UsersResources[0].find((resource) => {
        ...
        if (
        ...
        } else if (FoundExistingUserResource && Object.is(FoundExistingUserResource, FoundExistingUserResource)) {
            this.response
            .status(400)
            ...
            .status(400)
            .jsonp(
                "body" in request ? { Message: `Invalid email address, please include ${JSON.stringify('@gmail.com')} in your email!` } : ""
            );
        } else {
            // send successful response on successful resource creation
            return this.response.status(Number(parseInt(201))).json(
...
router.route("/:id").get(async (request, response) => {ReadResource(request, response, "Users_Resources")}).put(async (request, response) => {
      this.response = response;
      ...
      try {
        ...
        if (request && !FoundExistingUserResource || typeof FoundExistingUserResource === "undefined") {
          return this.response
            .status(404)
            .json({
        ...
        } else if(typeof request.body.first_name === "undefined" || typeof request.body.last_name === "undefined" || typeof request.body.email === "undefined" || typeof request.body.job === "undefined") {
                  return this.response.status(400).sendStatus(400);
          } else {
          ...
          return this.response
            .status(200)
            .jsonp(
              "body" in request ? { Message: `Updated resource with id ${FoundExistingUserResource.id}` } : ""
            );
        }
      } catch (error) {
        // console.log(error);
    ...
    }).delete(async (request, response) => { DeleteResource(request, response, "Users_Resources")});
